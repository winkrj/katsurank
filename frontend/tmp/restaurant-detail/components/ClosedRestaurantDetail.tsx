...
export function ClosedRestaurantDetail({ restaurant, layout }: ClosedRestaurantDetailProps) {
  if (layout === 'mobile') {
    return (
...
function ClosedActions({ layout }: { layout: 'desktop' | 'mobile' }) {
  const isMobile = layout === 'mobile'
  const className = isMobile ? 'grid grid-cols-2 gap-2' : 'flex max-w-[480px] gap-3'

  return (
    <div className={className}>
      <Link
        to="/rankings"
        className="flex h-12 flex-1 items-center justify-center gap-1.5 rounded-lg border-2 border-[#DBBA24] bg-[#FFC533] text-[14px] font-black text-[#2A1A12] transition hover:bg-[#D88A24]"
      >
        <TrophyIcon />
        랭킹 보기
      </Link>
      <Link
        to="/search"
        className="flex h-12 flex-1 items-center justify-center gap-1.5 rounded-lg border-2 border-[#E8D9BF] bg-white text-[14px] font-black text-[#2A1A12] transition hover:bg-[#FFF4D8]"
      >
        <PinIcon />
        다른 가게 찾기
      </Link>
  ...
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <path
        ...
        strokeLinecap="round"
      />
      <path d="M12 14v3M9 20h6" stroke="#2A1A12" strokeWidth="1.8" strokeLinecap="round" />
      <path d="M8 17h8" stroke="#2A1A12" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  )
}
