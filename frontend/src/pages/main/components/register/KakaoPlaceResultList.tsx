...
type KakaoPlaceResultListProps = {
  ...
  onViewRegistered: (id: number) => void
}

export function KakaoPlaceResultList({ places, onSelect, onViewRegistered }: KakaoPlaceResultListProps) {
  if (places.length === 0) {
    return (
            ...
            <div className="rounded-xl border border-[#E8D9BF] bg-[#FFFDF4] p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {place.isRegistered && place.registeredRestaurantId ? (
                  <button
                    type="button"
                    onClick={() => onViewRegistered(place.registeredRestaurantId as number)}
