    ...
    return (
      <li className="flex items-center gap-5 border-b border-[#F0E3CC] px-6 py-4 last:border-b-0 hover:bg-[#FFFBF0]">
        ...
        <div className="shrink-0 text-right">
          ...
          <p className={['mt-0.5 text-[13px] font-bold', isUp ? 'text-[#E05050]' : 'text-[#4A90D9]'].join(' ')}>
            지난주 대비 {isUp ? '▲' : '▼'} {Math.abs(item.weeklyDelta)}
          </p>
        </div>
    ...
    <li className="border-b border-[#F0E3CC] last:border-b-0">
      ...
      >
        ...
        <div className="shrink-0 text-right">
          ...
          <p className={['text-[11px] font-bold', isUp ? 'text-[#E05050]' : 'text-[#4A90D9]'].join(' ')}>
            {isUp ? '▲' : '▼'} {Math.abs(item.weeklyDelta)}
          </p>
        </div>
...
function RankMedal({ rank, size = 'md' }: { rank: number; size?: 'sm' | 'md' }) {
  ...
  if (rank === 1) {
    return (
      <span className={`flex ${dim} items-center justify-center rounded-full border-2 border-[#C78316] bg-[#FFC533] font-black text-[#3A2318]`}>
        {rank}
      </span>
    )
  }
  if (rank === 2) {
    return (
      <span className={`flex ${dim} items-center justify-center rounded-full border-2 border-[#9EA8B3] bg-[#C8D4DC] font-black text-white`}>
        {rank}
      </span>
    )
  }
  if (rank === 3) {
    return (
      <span className={`flex ${dim} items-center justify-center rounded-full border-2 border-[#B97830] bg-[#D99A4E] font-black text-white`}>
        {rank}
      </span>
    )
  }
  return (
    <span className={`flex ${dim} items-center justify-center font-black text-[#5F4A3C]`}>
      {rank}
    </span>
  )
}
