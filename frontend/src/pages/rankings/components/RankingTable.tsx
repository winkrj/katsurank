...
export function RankingTable({ items, layout }: RankingTableProps) {
  const [expanded, setExpanded] = useState(false)

  const visible = expanded ? items : items.slice(0, RANKING_PREVIEW_COUNT)
  const hiddenCount = items.length - RANKING_PREVIEW_COUNT
    ...
    <div className="overflow-hidden rounded-xl border border-[#E6D5B8] bg-white shadow-[0_4px_16px_rgba(58,35,24,0.06)]">
      ...
      {!expanded && hiddenCount > 0 && (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 border-t border-[#E8D9BF] py-4 text-[14px] font-bold text-[#5F4A3C] transition hover:bg-[#FFF4D8]"
        >
          더 많은 랭킹 보기 ({RANKING_PREVIEW_COUNT + 1} ~ {items.length}위)
          <ChevronDownIcon />
        </button>
      )}
    </div>
