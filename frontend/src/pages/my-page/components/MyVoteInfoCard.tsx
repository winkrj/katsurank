      ...
      {isDesktop ? (
        ...
        <>
          ...
          <p className="mb-4 text-[12px] text-[#5F4A3C]">
            내 표는 언제든 변경할 수 있어요. 새로운 가게를 찾아 응원해 주세요!
          </p>
          <div className="flex justify-around">
            {INFO_ITEMS.map(({ icon, label }) => (
              <div key={label} className="flex flex-col items-center gap-1.5 text-center">
                <span className="text-[#8A7A6A]">{icon}</span>
                <span className="text-[11px] font-bold leading-tight text-[#5F4A3C]">
...
function CalendarCheckIcon() {
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" aria-hidden>
      <rect x="4" y="8" width="28" height="24" rx="3" stroke="#D88A24" strokeWidth="2" fill="#FFF4D0" />
      <path d="M4 15h28" stroke="#D88A24" strokeWidth="2" />
      <path d="M12 4v6M24 4v6" stroke="#D88A24" strokeWidth="2" strokeLinecap="round" />
      <path d="M12 23l4 4 8-8" stroke="#D88A24" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
