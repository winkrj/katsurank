...
| M2 | 서버 배포 (백엔드) + CI/CD | ✅ 완료 | 2026-07-15 |
| M3 | 프론트엔드 개발 | 🚧 진행 중 | - |
| M4 | 프론트엔드 배포 | ⬜ 대기 | - |
| M5 | 데이터 시딩 (가게 20곳) | ✅ 완료 | 2026-07-17 |
| M6 | 소프트 런치 | ⬜ 대기 | - |
...
### 5단계 — CI/CD
- [x] GitHub Secrets 3개 등록(`PROD_HOST`/`PROD_USER`/`PROD_SSH_KEY` — 14개는 과다 산정이었음, `06_deployment_guide.md` v0.3 정정 참조) — 2026-07-15
- [x] 첫 수동 배포 + 헬스체크 확인
- [x] deploy.yml push → Actions 자동 실행 확인 — 2026-07-15

### 8단계 — PR 게이트 (추가, M2와 별개로 이후 진행)
- [x] `backend-test.yml`: PR에서 `./gradlew test`(ArchUnit 포함) 자동 실행, `backend/**` 미변경 PR은 skip 처리해 머지 안 막음 — 2026-08-05
- [x] `main` 브랜치 보호 규칙: `Test` 체크 필수 + `enforce_admins`(관리자 포함 직접 push 차단). approve 요구는 안 걺(백엔드 단독 담당이라 리뷰어 부재) — 2026-08-05
...
- [ ] Vercel 배포 (도메인: `www.katsurank.kr`)
- [ ] 환경변수 설정
- [x] CORS 설정 완료 (2026-07-17 확인) — 서버 `/etc/katsurank/env`의 `APP_CORS_ALLOWED_ORIGINS`에
      `http://localhost:5173,https://www.katsurank.kr` 이미 반영돼 있음. 실제 배포 도메인이
      `www.katsurank.kr`과 다르면(예: 커스텀 도메인 연결 전 임시 `*.vercel.app` 주소) 그때 추가 필요

---

## M5. 데이터 시딩 ✅ (2026-07-17)

> 계획했던 "SQL INSERT 스크립트"가 아니라, 실제 등록 API(`POST /api/v1/restaurants`)를
> 통해 시딩함 — CategoryValidator·SeoulAddressValidator 검증을 그대로 통과시켜서
> 실제 사용자가 등록하는 것과 동일한 경로로 데이터 정합성을 보장.

- [x] 서울 돈까스 맛집 20곳 선별 (카카오 로컬 API로 10개 구에 걸쳐 검색)
- [x] ~~SQL INSERT 스크립트 작성~~ → 등록 API로 대체
- [x] 카카오맵 카테고리·주소 기준 검증 (서버단 서울/돈까스 필터가 이미 검증)
- [x] 프로덕션에 20곳 등록 완료 (id 1~20, 전부 ACTIVE)
...
| 프론트 배포 위치 | ❓ 호스팅만 미정 | 도메인은 `www.katsurank.kr`로 이미 CORS에 반영됨(2026-07-17 확인). Vercel 등 호스팅 플랫폼 자체만 미정 |
...
- **v0.6 (2026-07-17)**: M5(데이터 시딩) 완료 — 계획했던 SQL 스크립트 대신 등록 API로 서울 10개 구 20곳 시딩. 이 과정에서 카카오 장소 검색 프록시에 서울/돈까스 필터·정확한 페이지네이션·캐싱 버그 수정을 함께 반영(PR #19~#21). 랭킹 조회 limit 상한도 50→100으로 상향(PR #23), 투표 API Swagger CSRF 헤더 노출 수정(PR #22). M4 CORS 설정 상태 확인 — 서버에 이미 `APP_CORS_ALLOWED_ORIGINS=http://localhost:5173,https://www.katsurank.kr` 반영돼 있음을 SSH로 확인, "미정"이던 프론트 배포 도메인이 사실상 `www.katsurank.kr`로 결정된 상태였음을 발견.
- **v0.7 (2026-08-05)**: 최상단 마일스톤 표(M2 "자동 배포는 다음 단계")·M2 5단계 체크리스트(`[ ]` 미체크)가 v0.5에서 이미 완료로 기록한 내용과 서로 모순돼 있던 걸 발견해 정정 — 체크리스트를 `[x]`로, 표 문구를 "완료"로 통일. 겸해서 PR 게이트 신설(8단계로 기록): `backend-test.yml`(PR 트리거 백엔드 테스트, `backend/**` 미변경 PR은 skip)과 `main` 브랜치 보호 규칙(`Test` 체크 필수 + `enforce_admins`)을 추가하고 실제 PR(#28~#30)로 동작 검증 완료.
