...
public class VoteService {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("투표 충돌 재시도 userId={} restaurantId={} attempt={} cause={}",
                            userId, restaurantId, attempt, ex.getClass().getSimpleName());
                }
                backoff(attempt);
            }
        }
    ...
    private VoteResponse doVote(Long userId, Long targetRestaurantId) {
        ...
        if (current != null && current.getRestaurantId().equals(targetRestaurantId)) {
            return VoteResponse.of(current, target); // 같은 가게 재투표 → 멱등(변화 없음)
        }
        ...
        Vote newVote = voteRepository.save(Vote.cast(userId, targetRestaurantId));
        // (5) 새 가게 표수 +1
