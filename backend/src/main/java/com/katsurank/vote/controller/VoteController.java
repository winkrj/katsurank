package com.katsurank.vote.controller;

import com.katsurank.vote.service.VoteService;

import com.katsurank.vote.dto.VoteRequest;
...
public class VoteController {
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "투표(이동) 성공")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "로그인 필요")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "존재하지 않는 restaurantId",
            content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "ACTIVE 가 아닌 가게에 투표 시도(RESTAURANT_NOT_VOTABLE) 또는 동시성 경합 재시도 초과(VOTE_CONFLICT)",
            content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    ...
    public ApiResponse<VoteResponse> vote(@Valid @RequestBody VoteRequest request,
                                          @LoginUser AuthPrincipal principal) {
        return ApiResponse.success(voteService.vote(principal.userId(), request.restaurantId()));
    }
}
