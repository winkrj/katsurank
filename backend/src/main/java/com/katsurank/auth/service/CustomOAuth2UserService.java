package com.katsurank.auth.service;

import com.katsurank.auth.dto.KakaoUserInfo;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public CustomOAuth2UserService(UserRepository userRepository, Clock clock) {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) {
        ...
        User user = userRepository.findByKakaoId(info.kakaoId())
                .map(existing -> {
                    existing.login(info.nickname(), info.profileImage(), now);
                    return existing;
                })
                .orElseGet(() -> {
                    log.atInfo().addKeyValue("kakaoId", info.kakaoId()).log("신규 사용자 가입");
                    try {
                        return userRepository.saveAndFlush(
                                User.register(info.kakaoId(), info.nickname(), info.profileImage(), now));
